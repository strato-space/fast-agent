...
def _prompt_command_completions(
    ...
    if not text_lower.startswith("/prompt load "):
        return None
...
def _skills_command_completions(
    completer: "AgentCompleter",
    text: str,
    text_lower: str,
) -> list[Completion] | None:
    if not text_lower.startswith("/skills "):
        return None
...
def _cards_publish_completions(
    ...
    if current_token.startswith("--") or not argument.strip():
        publish_flags = {
            ...
            "--message": "set publish commit message",
            "--temp-dir": "set parent directory for temp clone",
            "--keep-temp": "retain temp clone after publish",
        }
        results.extend(
            Completion(
                flag,
            ...
            for flag, description in publish_flags.items()
...
def _cards_command_completions(
    completer: "AgentCompleter",
    text: str,
    text_lower: str,
) -> list[Completion] | None:
    if not text_lower.startswith("/cards "):
        return None
...
def _model_catalog_completions(argument: str, results: list[Completion]) -> list[Completion]:
    ...
    if ((not current_token and argument.endswith(" ")) or current_token.startswith("--")) and (
        "--all".startswith(current_token.lower())
    ):
        results.append(
            Completion(
                "--all",
    ...
    if subcmd == "clear":
        ...
        return [
            Completion(
                ...
                display=option,
                display_meta=(
                    "clear full session store" if option == "--all" else "attached mcp server"
                ),
            )
...
def command_completions(
    ...
    history_result = _history_command_completions(completer, text, text_lower)
    ...
    skills_result = _skills_command_completions(completer, text, text_lower)
    ...
    mcp_result = _mcp_command_completions(completer, text, text_lower)
    if mcp_result is not None:
        return mcp_result

    card_result = _card_command_completions(completer, text_lower, text)
    if card_result is not None:
        return card_result

    agent_result = _agent_command_completions(completer, text_lower, text)
