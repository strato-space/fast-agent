...
This document is an audit against the GitHub release text for `v0.4.40`:
...
## Potential bugs spotted in the code (v0.4.37 -> latest upstream)

These are code-level issues noticed while diffing `v0.4.37` -> `cb9dd0f5` (independent of the release notes mismatch).

### 1) Anthropic reasoning budget presets advertise `0`, but validation rejects it

In `src/fast_agent/llm/model_database.py`, the Anthropics "thinking" reasoning spec includes `budget_presets=[0, 1024, 16000, 32000]` while also setting `min_budget_tokens=1024`.
However, `validate_reasoning_setting()` enforces the `min_budget_tokens` lower bound for budget settings, so `0` is treated as invalid.

Practical impact:
- The UI/CLI can present `0` as an allowed value (because it comes from `budget_presets`),
  but setting `reasoning=0` errors with "Budget must be >= 1024 tokens."

Likely fix direction (pick one):
- Allow `budget=0` as a special-case "off" for budget-based specs, OR
- Remove `0` from the preset list and rely on `off` for disabling reasoning.

### 2) Duplicate skills tool-call branch in MCP agent server extraction

In `src/fast_agent/agents/mcp_agent.py`, the skills-reader tool-call detection block appears **twice** in a row inside `_extract_servers_from_message()`.

Impact:
- Probably harmless at runtime, but it is a clear copy/paste defect and increases the risk of future regressions.
...
This might be intentional aliasing, but it’s unusual and worth confirming.
It also looks intentional (not a mistake):
